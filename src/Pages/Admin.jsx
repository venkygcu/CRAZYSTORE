...
const Admin = () => {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      setProductError(e?.response?.data?.message || 'Failed to load products');
    } finally {
            ...
            {loadingProducts ? (
              ...
              <div style={{ display: 'grid', gap: 12 }}>
                ...
                {products.map((product) => (
                  <div key={product.id} style={{ border: '1px solid #ddd', borderRadius: 6, padding: 12, display: 'flex', gap: 16 }}>
                    <img src={product.image} alt={product.name} style={{ width: 80, height: 80, objectFit: 'cover', borderRadius: 6 }} />
                    <div>
                      <h3 style={{ margin: '0 0 4px' }}>{product.name}</h3>
                      <p style={{ margin: 0, color: '#555' }}>Category: {product.category}</p>
                      <p style={{ margin: '4px 0' }}>₹{product.new_price}{product.old_price ? <span style={{ textDecoration: 'line-through', marginLeft: 8, color: '#999' }}>₹{product.old_price}</span> : null}</p>
                      <small style={{ color: '#777' }}>{new Date(product.created_at).toLocaleString()}</small>
                    </div>
                  </div>
                ))}
              </div>
